...
df = pd.read_csv(r'AAL90_Binary.csv', sep=',').values
...
plt.subplots_adjust(left=0.12,
                ...
                right=0.98, 
                top=0.98)
plt.gcf().text(0.25, 0.91, r"$\tilde{k}_1$" + f"$={str(k)}$", fontsize=15, fontweight="bold")
plt.gcf().text(0.32, 0.475, r"$\tilde{k}_3$" + f"$={str(k3)}$", fontsize=15, fontweight="bold")
ax.arrow(k+1.5,max_gap,5,0, head_width = 0.005, head_length=2, color='red')
ax.arrow(k3+1.5,max_gap_3,5,0, head_width = 0.005, head_length=2, color='blue')
...
ax.vlines(k3, ymin=0, ymax=max_gap_3-0.012, linewidth=0.75, linestyle='dashed', color='blue')

ax.spines['right'].set_visible(False), ax.spines['top'].set_visible(False)
ax.set_ylabel("$\lambda_{k+1} - \lambda_k$", fontsize=14), ax.set_xlabel("k", fontsize=14)
ax.set_xticks([1,20,40,60,80]), ax.set_xlim([0.5,eigenvalues.size+1]), ax.set_ylim([0,np.max(gap)+0.01])

left, bottom, width, height = [0.6, 0.6, 0.36, 0.36]
...
inset.scatter(k3, eigenvalues[k3-1], s=4, color='blue')
inset.vlines(k, ymin=0, ymax=eigenvalues[k-1]-0.012, linewidth=0.5, linestyle='dashed', color='red')
inset.vlines(k3, ymin=0, ymax=eigenvalues[k3-1]-0.012, linewidth=0.5, linestyle='dashed', color='blue')
...
inset.set_xticks([1,20,40,60,80]), inset.set_xlim([0,eigenvalues.size+1]), inset.set_ylim([0,np.max(eigenvalues)+0.1])

plt.savefig("Eigengap_AAL90.png", dpi=900)